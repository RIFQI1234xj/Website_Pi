...
import { ArrowLeft, Loader2, Book, ChevronLeft, ChevronRight, Share2 } from 'lucide-react';
...
export const ProgramDetail: React.FC = () => {
  ...
  const programId = programIdParam ? Number(programIdParam) : NaN;
  ...
  useEffect(() => {
    ...
    apiFetch('/programs')
      .then((result) => {
        if (result.success) {
          const item = result.data.find((p: ProgramData) => p.id === programId);
          if (item) {
            setProgram(item);
          } else {
            setIsError(true);
      ...
      .catch((err) => {
        console.error("Error mengambil detail program:", err);
        setIsError(true);
      })
      .finally(() => setIsLoading(false));
  }, [programId]);

  const getPhotos = (item: ProgramData): string[] => {
    if (item.images && Array.isArray(item.images) && item.images.length > 0) {
      const validPhotos = item.images.filter((photo) => !isDummyImageSource(photo));
  ...
  if (isError || !program) {
    return (
      <div className="bg-ivory-50 min-h-screen">
        <div className="bg-teal-700 pt-32 pb-16 text-center text-white">
          <h1 className="font-serif text-4xl font-bold mb-4">Program Tidak Ditemukan</h1>
        </div>
        <div className="max-w-4xl mx-auto px-4 py-12 text-center">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12">
            <Book size={64} className="text-gray-300 mx-auto mb-6" />
            <h2 className="text-2xl font-bold text-gray-800 mb-3">Oops!</h2>
            <p className="text-gray-500 mb-8 max-w-md mx-auto">
              Program yang Anda cari mungkin sudah dihapus atau alamatnya salah.
            </p>
            <Link
              to="/program"
  ...
  const photos = getPhotos(program);
              ...
              <div className="relative w-full rounded-2xl overflow-hidden shadow-sm bg-gray-100 mb-6 group flex items-center justify-center min-h-[200px]">
                {photos.map((photo, index) => (
                  <img
                    ...
                    onError={(e) => setImageFallback(e.currentTarget, program.title)}
                  />
                ))}
                ...
                {photos.map((photo, index) => (
                  ...
                  >
                    <img
                      ...
                      onError={(e) => setImageFallback(e.currentTarget, `Thumbnail ${program.title}`)}
                    />
                  </button>
                ))}
              </div>
